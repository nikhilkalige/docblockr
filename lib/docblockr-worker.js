...
module.exports =
  DocBlockrAtom = (function() {
    ...
    function DocBlockrAtom() {
      ...
      atom.workspaceView.command('docblockr:parse-tab', function(event) {
        var regex = {
          // Parse Command
          'parse': /^\s*(\/\*|###)[*!]\s*$/,
          // Indent Command
        ...
        // Parse Command
        if(self.validate_request(true, regex.parse)) {
          console.log('Parse command');
          self.parse_command(false);
        }

        // Indent Command
        else if(self.validate_request(true, regex.indent)) {
      ...
      atom.workspaceView.command('docblockr:parse-enter', function(event) {
        var regex = {
          // Parse Command
          'parse': /^\s*(\/\*|###)[*!]\s*$/,
        ...
        // Trim auto whitespace
        else if(self.validate_request(true, regex.trim_auto[0], true, regex.trim_auto[1])) {
          console.log('Auto Trim command');
          self.trim_auto_whitespace_command();
        }
        // Deindent command
        else if(self.validate_request(true, regex.deindent)) {
          console.log('Deindent command');
          self.deindent_command();
        }

        // Extend docblock by adding an asterix at start
        else if(self.validate_request(true, regex.extend)) {
          ...
          preceding_text = preceding_text.replace(_regex, '$1');
          editor.insertText('\n' + preceding_text);
        }
      ...
      atom.workspaceView.command('docblockr:parse-inline', function(event) {
        console.log('Parse-Inline command');
        var _regex = /^\s*\/\*{2}$/;
        
        if(self.validate_request(true, _regex))
          self.parse_command(true);
        else
          event.abortKeyBinding();
      });

      atom.workspaceView.command('docblockr:join', function(event) {
        console.log('Join command');
        self.join_command();
      });

      atom.workspaceView.command('docblockr:reparse', function(event) {
        console.log('Reparse command');
        self.reparse_command();
      });

      atom.workspaceView.command('docblockr:wrap_lines', function(event) {
        console.log('Wraplines command');
        self.wrap_lines_command();
      });
    }

    DocBlockrAtom.prototype.validate_request = function(preceding, preceding_regex, following, following_regex) {
      /**
      ...
      preceding = (typeof preceding !== 'undefined') ? preceding : false;
      following = (typeof following !== 'undefined') ? following : false;
...
class JsdocsDecorateCommand(sublime_plugin.TextCommand):
    ...
    DocBlockrAtom.prototype.reparse_command = function() {
      ...
      var scope_range = this.scope_range(editor, pos);
    ...
    DocBlockrAtom.prototype.scope_range = function(editor, point) {
      // find scope starting point
      // checks: ends when row less than zero, column != 0
      // check if current point is valid 
      var _range;
      if((_range = editor.displayBuffer.bufferRangeForScopeAtPosition('.comment', point)) == null)
        return null;
      ...
      while(_row >= 0) {
        ...
        _range = editor.displayBuffer.bufferRangeForScopeAtPosition('.comment', [_row, line_length]);
        if(_range == null)
          break;
        start = _range.start;
        if(start.column > 0) {
      ...
      while(_row < last_row) {
        line_length = editor.lineLengthForBufferRow(_row);
        _range = editor.displayBuffer.bufferRangeForScopeAtPosition('.comment', [_row, 0]);
        if(_range == null)
          break;
        end = _range.end;
