  ...
  DocBlockrAtom = (function() {
    ...
    function DocBlockrAtom() {
      ...
      atom.workspaceView.command('docblockr:parse-tab', function(event) {
        ...
        // Parse Command
        if(self.validate_request({preceding:true, preceding_regexregex.parse})) {
          // console.log('Parse command');
          self.parse_command(false);
        }

        // Indent Command
        else if(self.validate_request({preceding:true, preceding_regexregex.indent})) {
      ...
      atom.workspaceView.command('docblockr:parse-enter', function(event) {
        ...
        // Trim auto whitespace
        else if(self.validate_request({preceding:true, preceding_regex:regex.trim_auto[0], following:true, following_regex:regex.trim_auto[1]})) {
        ...
        else if(self.validate_request({preceding:true, preceding_regex:regex.extend_line})) {
        ...
        // Extend docblock by adding an asterix at start
        else if(self.validate_request({preceding:true, preceding_regex:regex.extend})) {
