...
module.exports =
  DocBlockrAtom = (function() {
    ...
    function DocBlockrAtom() {
      ...
      atom.workspaceView.command('docblockr:decorate_multiline', function(event) {
        // console.log('Decorate Multiline command');
        if(self.validate_request({scope:'comment.block'}))
          self.decorate_command_multiline();
      });
...
DocBlockrAtom.prototype.decorate_command_multiline = function() {
  var editor = atom.workspace.getActiveEditor();
  var pos = editor.getCursorBufferPosition();
  var whitespace_re = /^(\s*)\/\*/;
  ...
  _firstElement = [[scope_range[0].row + 1, 0], [scope_range[0].row + 1, 1]];

  //console.log('_len ' + _len);

  //console.log(scope_range);
  ...
  for(_i = _len; _i >= 0; _i--) {
    line_text = editor.lineForBufferRow(_row + _i);
//    editor.setTextInBufferRange(_firstElement,'*');
    var _length = editor.lineLengthForBufferRow(_row + _i);
    var r_padding = 1 + (max_len - _length);
    var _range = [[scope_range[0].row + _i, 0], [scope_range[0].row + _i, _length]];
    editor.setTextInBufferRange(_range, leading_ws + line_text + this.repeat(' ', r_padding+3) + '*');
  }
  editor.buffer.insert(scope_range[0], '/*' + this.repeat('*', line_length + 3) + '\n');
