...
module.exports =
	DocBlockrAtom = (function () {
		 ...
		 * One config wathcer
		 * Three Keybound commands (tab, enter, shift+enter)
		 * Five Supporting commands (join, reparse, wrap, decorate)
		 * 
		 * @returns {undefined}
		 */
		function DocBlockrAtom() {
			...
			// When we press Tab, we 
			//   Parse 
			//   Indent
			//
			// When we press Enter, we 
			//   Parse 
			//   Trim 
			//   DeIndent
			//   Write(Snippet)
			...
			//   Write(Extend Block)
			//
			// When we press Shift+Enter we
			//   Parse-Inline
			//   Write(Newline)

			/** 
			 * Defines what happens when the Tab key is pressed
			 * @param {event} takes in the event, the tab key press event
			 * @returns {undefined}
			 */
			atom.commands.add('atom-workspace', 'docblockr:parse-tab', function (event) {
				...
				// Indent Command
				else if (self.validate_request({
						preceding: true,
						preceding_regex: regex.indent
					})) {
			...
			// *  Comment Extension: Pressing enter inside a docblock will automatically insert a leading asterisk and maintain your indentation.
			 ...
			 * @param {event} takes in the event, the tab Enter press event
			 * @returns {undefined}
			 */
			atom.commands.add('atom-workspace', 'docblockr:parse-enter', function (event) {
				...
				// Trim auto whitespace
						...
						preceding: true,
						preceding_regex: regex.trim_auto[0],
						following: true,
						following_regex: regex.trim_auto[1],
						scope: 'comment.block'
					})) {
				...
				// Deindent command
						...
						preceding: true,
						preceding_regex: regex.deindent
					})) {
				...
				// Snippet-1
						...
						preceding: true,
						preceding_regex: regex.snippet_1[0],
						following: true,
						following_regex: regex.snippet_1[1]
					})) {
					// console.log('Snippet-1 command');
					var editor = atom.workspace.getActiveTextEditor();
					self.write(editor, '\n$0\n ');
				}
				// Close block comment (Parse Block)
				else if (self.validate_request({
						preceding: true,
						preceding_regex: regex.close_block
					})) {
					// console.log('Snippet close block comment command');
					self.parse_block_command();
				}
				// extend line comments (// and #)
				else if ((self.editor_settings.extend_double_slash == true) && (self.validate_request({
						preceding: true,
						preceding_regex: regex.extend_line,
						scope: 'comment.line'
					}))) {
			...
			// If the line following your docblockr contains a variable declaration, 
			// DocBlockr will try to determine the data type of the variable and insert that into the comment.
			/**
			 * Defines what happens when the  Shift + Enter key is pressed
			 * @param {event} takes in the event, the tab  Shift + Enter press event
			 * @returns {undefined}
			 */
			...
			/**
			 * Joins some lines of comments into a single line
			 * @param {event} 
			 * @returns {undefined}
			 */
			atom.commands.add('atom-workspace', 'docblockr:join', function (event) {
				self.event = event;
				// Only execute this command if we are in the context of a block comment
				if (self.validate_request({
						scope: 'comment.block'
					}))
					self.join_command();
			...
			// ~~~~ FEATURES ~~~~
			// Sometimes, you'll perform some action which clears the fields (sections of text which you can navigate through using tab). 
			// This leaves you with a number of placeholders in the DocBlock with no easy way to jump to them.
			...
			// ~~~~ FEATURES ~~~~
			// Inside a comment block, hit Docblockr:Wrap Lines to wrap the lines to make them fit within your rulers.  
			// If you would like subsequent lines in a paragraph to be indented, you can adjust the indentation_spaces_same_para setting. 
			/**
			 * Wrap Lines Command
			 * @param {event} 
			 * @returns {undefined}
			...
			// It will basically add a bunch of stars to the top, bottom, and right side
			/**
		...
		/**
		 * Validate Request is used Filter which commands should be run.
		 * @param options
		 ...
		 * @returns {Boolean} returns if we should 
		 */
		DocBlockrAtom.prototype.validate_request = function (options) {
			...
			for (i = 0, len = cursor_positions.length; i < len; i++) {
				...
				if (preceding)
					preceding_text = editor.getTextInBufferRange([
						[cursor_position.row, 0], cursor_position
					]);
		...
		// 										Tab
		/**
		 * Parse Command is used by each of the three keybindings
		 * We First initialize the worker
		 * We then check to see if we are in a block comment
		 * If so, It then Writes a new line with another asterisk
		 * 		  Then indents it the number of spaces as the line above
		 * 		  And Returns
		 * if not, we generate an appropriate snippet and insert it 
		 * 
		 * It writes a snippet to the editor
		 * @param {any} inline
		 * @returns undefined
		 */
		DocBlockrAtom.prototype.parse_command = function (inline) {
			...
			// We Erase from the cursor to the end of the line.
		...
		/**
		 * Used Exclusively with Tab/Parse
		 * Inserts some spaces a certain number of times
		 * This provides the indent functionality when inserting a snippet
		 * @returns undefined
		 */
		DocBlockrAtom.prototype.indent_command = function () {
			var editor = atom.workspace.getActiveTextEditor();
			var current_pos = editor.getCursorBufferPosition();
			// Get the text of the line above the cursor
		...
		/**
		 * Get Indent Spaces
		 * Used Exclusively by the Indent_Command function
		 * @param {any} editor
		 * @param {any} line
		 * @returns {number} the number of spaces to indent the comment
		 */
		 ...
		 * Used Exclusively with the Parse-Enter command.  
		 * 
		 * @returns undefined
		 */
		DocBlockrAtom.prototype.trim_auto_whitespace_command = function () {
			...
			var range = [
				...
				[cursor_position.row, line_length]
			];
			editor.setTextInBufferRange(range, line_text);
		};
		/**
		 * Deindent a Comment Block
		 */
		 ...
		 * Used by the Parse-Enter command... 
		 * Writes a new line with containing asterisk
		 * Write indentation and trailing characters. Place cursor before trailing characters
		 *  
		 * @returns undefined
		 */
		...
		// =====================================================================================
		/**
		 * Join a number of lines in a block comment
		 * Only used with the Join command, only works in a Block comment
		 * @returns undefined
		 */
		DocBlockrAtom.prototype.join_command = function () {
			...
			for (i = 0; len = selections.length, i < len; i++) {
				...
				row_begin = Math.min(_r[0], _r[1]); // The first for in the selection
				if (no_rows === 0) {
				...
				var text = '';
				// Get the Text of the selected rows
				// Join the Commented lines into one string
				for (j = 0; j < no_rows; j++) {
					text += text_with_ending(row_begin + j);
				}
				var regex = /[ \t]*\n[ \t]*((?:\*|\/\/[!/]?|#)[ \t]*)?/g;
				text = text.replace(regex, ' ');
				// Get the length of the last line of the block comment
				var end_line_length = editor.lineTextForBufferRow(row_begin + no_rows - 1).length;
				var range = [
					[row_begin, 0],
					[row_begin + no_rows - 1, end_line_length]
				];
				// Put the Joined text into the last line
		...
		/**
		 * If you write a double-slash comment and then press Ctrl+Enter, DocBlockr will 'decorate' that line for you.
		 * Decorate, surrounds a double-slash comment with a box of slashes acting as a frame
		 * @returns {undefined} Inserts text 
		 */
		DocBlockrAtom.prototype.decorate_command = function () {
			...
			// RegExp
			var whitespace_re = /^(\s*)\/\//;
			// Get a Range for the current Scope
			...
			for (_i = _len; _i >= 0; _i--) {
				...
				var _range = [
					[scope_range[0].row + _i, 0],
					[scope_range[0].row + _i, _length]
				];
		...
		// =====================================================================================
		/**
		 * If you are in a block comment and want to box it in, use the decorate multiline command
		 * @returns {undefined} Inserts text 
		 */
		...
		// =====================================================================================
		/**
		 * ReParse to Reestablish Tab Stops
		 * Sometimes, you'll perform some action which clears the fields (sections of text which you can navigate through using Tab). 
		 * This leaves you with a number of placeholders in the DocBlock with no easy way to jump to them.
		 * With DocBlockr, you can reparse a comment and reactivate the fields 
		 *  by pressing the hotkey Alt+Shift+Tab in OS X or Linux, or Alt+W in Windows
		 * @returns undefined
		 */
		...
		// =====================================================================================
		/**
		 * Format a long comment line into a multiline comment, with subsequent lines optionally indented
		...
		// =====================================================================================
		// ==========     Initialize, Get Parser, Get Scope, Write, Erase     ==================
		// =====================================================================================
		...
		DocBlockrAtom.prototype.initialize = function (editor, inline) {
			...
			// Get the proper parser from the current editor, this will be something like Java or c++
		...
		/**
		 * Here is where we return a certain parser, based on the configuration settings
		 * We check the language of the current editor
		 * We then return the parser from the languages folder
		 * @param {any} editor
		 * @returns
		 */
		DocBlockrAtom.prototype.get_parser = function (editor) {
			...
			var settings = atom.config.get('docblockr');
			// Return a parser, with the options... 
		...
		/**
		 * Get a Range for a certaiin scope
		 * Returns the start and end positions of the range as an array
		 * @param {any} editor
		 * @param {any} point
		 * @param {any} scope_name
		 * @returns {position[]} [start, end]
		 */
		...
		/**
		 * Write a Code Snippet to the editor, this uses the Atom Snippet service
		 * Used with three Parse/Reparse and two Decorate commands
		 * @param {TextEditor} editor to use to insert text
		 * @param {String} Snippet String to insert
		 */
		 ...
		 * Used by the two Parse commands
		 * @param {any} editor
		 * @param {any} range
		 */
		...
		// =====================================================================================
		// ======================     Supporting and Unused    =================================
		// =====================================================================================
		/**
		 * Utility function to multiply a string a certain number of times
		 * Used everywhere (18 places)
		...
		/**
		 * Update the Configuration in this class based on the workspace config
		 * @returns {undefined}
		 */
		...
		// =====================================================================================
		/**
		 * Used exclusively by the Parse_Command function
		 * @param {any} out
		 * @param {any} inline
		 * @returns {snippet} a Snippet to insert
		 */
		...
		/**
		 * Fills an array with 0s a certain lengh
		 * Used with Align Tags
		 * @param {any} len
		 * @returns {number[]}
		 */
