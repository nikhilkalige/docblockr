...
module.exports =
  DocBlockrAtom = (function() {
    ...
    function DocBlockrAtom() {
      ...
      atom.workspaceView.command('docblockr:wrap-lines-generic', function(event) {
        // console.log('Wrap Lines command');
          self.wrap_generic();
      });
    }
    ...
    DocBlockrAtom.prototype.wrap_generic = function() {
        var editor = atom.workspace.getActiveEditor();
        var wrap_len = atom.config.get('editor.preferredLineLength');
	  	  var _range = editor.getSelectedBufferRange();
        var sel_line = editor.getTextInBufferRange(_range);
        var _i, _len;

        var num_of_begin_spaces = 0;

        var words_w = sel_line.split('');
        var i=0;
        //count number of white spaces at the beginning
        while(words_w[i] ===' ')
         {
          num_of_begin_spaces++;
          i++;
        ...
        var _words = sel_line.trim().split(' ');

        i=0;
        var words = [];
        var blankW = '';
        while(i<num_of_begin_spaces-1){
          blankW = blankW + ' ';
          i++;
        }
        if(num_of_begin_spaces)
          words[0] = blankW;
        ...
        editor.setTextInBufferRange(_range,full);
