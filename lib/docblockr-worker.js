...
var DocsParser = require('../index.js');
var Snippets = atom.packages.activePackages.snippets.mainModule;
//var snippet = require('/Users/lonewolf/Downloads/Atom.app/Contents/Resources/app/node_modules/snippets/lib/snippets');
var DocBlockrAtom;
module.exports =
  // Class docblockatom
  DocBlockrAtom = (function() {
    ...
    DocBlockrAtom.prototype.align_tags = function(out) {
      ...
      for(var i=0; i<out.length; i++) {
        if(out[i].startsWith('@')) {
          ...
          maxCols = Math.max(maxCols, widths[-1].length);
        }
      }
      // initialise a list to 0
      var maxWidths = [0] * maxCols;
    ...
    DocBlockrAtom.prototype.fix_tab_stops = function(out) {
      ...
      for(i=0; len = out.length, i<len; i++)
        out[i] = out[i].replace(/(\$\{)\d+(:[^}]+\})/, swap_tabs);
      return out;
    }
