...
var Parsers = {
  ...
  PythonParser: require('./languages/python')
};
  ...
  DocBlockrAtom = (function() {
    ...
    function DocBlockrAtom() {
      ...
      atom.commands.add('atom-workspace', 'docblockr:parse-tab', function(event) {
        ...
        if(self.validate_request({preceding:true, preceding_regex:regex.parse})) {
          ...
          this.parser.parse_command(false);
        }
      ...
      atom.commands.add('atom-workspace', 'docblockr:parse-enter', function(event) {
        ...
        self.initialize(editor, false);
        var regex = self.parser.regex
        // Parse Command
        if(self.validate_request({preceding:true, preceding_regex:regex.parse})) {
          console.log('Parse command');
          self.parser.parse_command(false, editor, self);
        }
        // Parse Python Command
        else if(self.validate_request({preceding:true, preceding_regex:regex.parse_py})) {
          console.log('Parse python command');
          self.parse_python_command(false);
        }
        ...
        //Close python block docstring
        else if(self.validate_request({preceding:true, preceding_regex:regex.close_py_block})) {
          console.log('Snippet close block comment command');
          self.parse_py_block_command();
        }
      ...
      atom.commands.add('atom-workspace', 'docblockr:parse-inline', function(event) {
        ...
        if(self.validate_request({preceding:true, preceding_regex:_regex}))
          this.parser.parse_command(true);
        else {
          var editor = self.get_relevant_editor();
    ...
    DocBlockrAtom.prototype.validate_request = function(options) {
      ...
      for (i = 0, len = cursor_positions.length; i < len; i++) {
        ...
        console.log("Preceding : " + preceding_text);
    ...
    DocBlockrAtom.prototype.parse_python_command = function(inline) {
      ...
      matches = 0;
      line = this.read_line(editor, pos);
      ...
      var snippet = this.generate_python_snippet(out, inline);
    ...
    DocBlockrAtom.prototype.parse_py_block_command = function () {
      var editor = this.get_relevant_editor();
      if (typeof editor === 'undefined' || editor === null) {
        return;
      }
    ...
    DocBlockrAtom.prototype.initialize = function(editor, inline) {
      ...
      if (typeof this.get_parser(editor).settings.prefix !== 'undefined')
    ...
    DocBlockrAtom.prototype.get_parser = function(editor) {
      ...
      if(source_lang === 'coffee')
        ...
        return new Parsers.PythonParser(settings);
      return new Parsers.JsParser(settings);
    };

    DocBlockrAtom.prototype.generate_python_snippet = function(out, inline) {
