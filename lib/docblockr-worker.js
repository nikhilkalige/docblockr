...
module.exports =
  DocBlockrAtom = (function() {
    ...
    DocBlockrAtom.prototype.decorate_multiline_command = function() {
      ...
      // get max_len, ignore block indent
      for(_i = 1; _i < _len; _i++) {
        var text_length;
        line_text = editor.lineForBufferRow(_row + _i);
        //text_length = line_text.replace(/\s/g, '').length;
        //matches = /^\s*\*(.*)/.exec(line_text);
        //line_text = matches[1];
      ...
      line_text = line_text.replace(/^(\s*)(\*)+\//, function(self) {
        return function(match, p1, stars) {
          ...
          return (p1 + block_ws + self.repeat('*' , (line_length + 2 - len)) + '/' + '\n');
        }
      }(this));
      ...
      for(_i = _len-1; _i > 0; _i--) {
        ...
        editor.setTextInBufferRange(_range, block_ws + line_text + this.repeat(' ', r_padding) + '*');
      }
      //editor.buffer.insert(scope_range[0], '/*' + this.repeat('*', line_length + 3) + '\n');
    };
