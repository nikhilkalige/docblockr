...
DocsParser.prototype.parse_command = function(inline, editor, docblockratom) {
  if (typeof editor === 'undefined' || editor === null) {
    return;
  }
  console.log('line:'+docblockratom.line)
  if(this.is_existing_comment(docblockratom.line)) {
    docblockratom.write(editor, '\n *' + docblockratom.indentSpaces);
    return;
  }

  // erase characters in the view (will be added to the output later)
  docblockratom.erase(editor, docblockratom.trailing_range);

  // match against a function declaration.
  var out = this.parse(docblockratom.line);
  var snippet = this.generate_snippet(out, inline, docblockratom);
  // atom doesnt currently support, snippet end by default
  // so add $0
  if((snippet.search(/\${0:/) < 0) && (snippet.search(/\$0/) < 0))
    snippet+= '$0';
  docblockratom.write(editor, snippet);
};

DocsParser.prototype.generate_snippet = function(out, inline, docblockratom) {
  //# substitute any variables in the tags
  if(out)
    out = docblockratom.substitute_variables(out);

  // align the tags
  if(out && (docblockratom.shallowAlignTags || docblockratom.deepAlignTags) && (!inline))
    out = docblockratom.align_tags(out);

  // fix all the tab stops so they're consecutive
  if(out)
    out = docblockratom.fix_tab_stops(out);
  ...
  if(inline) {
    ...
    return (docblockratom.create_snippet(out) + (docblockratom.editor_settings.newline_after_block ? '\n' : ''));
};
