...
DocsParser.prototype.parse = function(line) {
    ...
    out = this.parse_getter(line); //  (name, retval)
    if (out) {
        return this.format_getter.apply(this, out)
    }
    return null;
};
...
DocsParser.prototype.format_getter = function(name, retval) {
    // I don't know, and don't have the energy to find out, what most of this does
    // For that reason I'm copying the format_function method, and changing what I care about
    // If you find this comment in a PR, please feel free to clean up this method before merging
    var out = [];
    var i, len, format_str;
    var extra_tag_after = this.editor_settings.extra_tags_go_after || false;
    ...
    if (ret_type !== null) {
        ...
        var format_args = [
            (this.editor_settings.type_tag || '@type'),
            type_info
        ];
