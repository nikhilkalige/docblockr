...
DocsParser.prototype.format_function = function(name, args, retval, options) {
    ...
    // If a function has "test" in his name, then
    // add @test and a @depends
    if(options.is_php){
        if(name.toLowerCase().search('test') > -1){
            out.push('@test '+ escape(name));
            if(args)
                ...
                var parsed_args = this.parse_args(args);
                for (i = 0; len = parsed_args.length,i < len; i++) {
                    out.push('@depends [dependFunction]');
                }
        }
    }
