...
ActionscriptParser.prototype.parse_function = function(line) {
    ...
    var name = matches[1] && (matches[1] || matches[3] || '').replace(regex, '\\1');
    var args = matches[4];
    var options = {};
    if(matches[2] == 'set')
...
ObjCParser.prototype.get_definition = function(editor, pos, read_line) {
    ...
    for(i=0; i < maxLines; i++) {
        ...
        line = line.replace(/\/\/.*/, '');
        if(definition === '') {
            if((!this.settings.fnOpener) || !(line.search(this.settings.fnOpener) > -1)) {
                definition = line;
                break;
            }
        }
...
ObjCParser.prototype.parse_function = function(line) {
    ...
    var matches = line.exec(regex);
    if(matches == null)
        return null;
    
    var name = matches[2];
    var arg_str = matches[3];
    var args = [];
    ...
    return [name, args.join('|||'), matches[1]];
};
...
JavaParser.prototype.parse_function = function(line) {
    ...
    var regex = new RegExp(
        ...
        // Method name
        '(' + this.settings.fnIdentifier + ')\\s*' + 
        // Params
        '\\((.*)\\)\\s*' + 
        // # Throws ,
        '(?:throws){0,1}\\s*([a-zA-Z_$0-9\\.,\\s]*)'
        );
    ...
    var name = matches[2];
    var retval = matches[1];
    var full_args = matches[3];
    var arg_throws = matches[4] || '';
...
JavaParser.prototype.format_function = function(name, args, retval, throws_args, options) {
    options = (typeof options !== 'undefined') ? options : {};
    var out = DocsParser.format_function.call(this, name, args, retval, options);
    if(throws_args !== '') {
    ...
    for(i=0; i < maxLines; i++) {
        ...
        if(definition === '') {
            ...
            // Handle Annotations
                ...
                continue;
            // Check for function
            else if(!(this.settings.fnOpener) || !(line.search(this.settings.fnOpener) > -1)) {
                definition = line;
                break;
            }
        }
...
TypescriptParser.prototype.setup_settings = function() {
    ...
    this.settings = {
        ...
        'functionRE':
            ...
            '\\((.*)\\)\\s*' + 
            // Return value
            '(:\\s*(' + parametric_type_identifier + '))?',
        'var_re':
            '((public|private|static|var)\\s+)?(' + identifier + 
            ')\\s*(:\\s*(' + parametric_type_identifier + 
            '))?(\\s*=\\s*(.*?))?([;,]|$)'
    };
    this.function_re = this.settings.function_re;
    this.var_re = this.settings.var_re;
};

TypescriptParser.prototype.parse_function = function(line) {
    line = line.trim();
    var matches = line.exec(this.function_re);
    if(matches !== null) 
        return null;
...
TypescriptParser.prototype.parse_var = function(line) {
    var matches = line.exec(this.var_re);
    if(matches == null) 
        return null;
    var val = matches[3];
    if(val != null)
    ...
    return [matches[1], val, matches[2]];
};
