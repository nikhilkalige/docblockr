...
module.exports = class PhpParser extends DocsParser {
  ...
  parseFunction (line) {
    const r = '^\\s*((public|protected|private|final|static)\\s+){0,3}' +
              'function\\s+&?(?:\\s+)?' +
    ...
    if (matches.retval) {
      retval = matches.retval.trim();
      
      if (retval.charAt(0) === '?') {
        retval = retval.substr(1) + '|null';
      }
    }

    return [matches.name, (matches.args ? matches.args.trim() : null), retval];
  }
